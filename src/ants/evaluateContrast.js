...
import { filter, findInObject, map, values } from 'rambdax'
...
export function evaluateContrast(){
  ...
  const toLog = []
  map(singleSetting => {
    const colors = filter((_, prop) => prop.startsWith('COLOR') && prop.length === 7)(singleSetting)
    ...
    const sum = values(sk).reduce((prev, curr) => curr + prev, 0)
  ...
  const minColor = findInObject(x => x === min, hash)
  const maxColor = findInObject(x => x === max, hash)
  console.log(hash)
